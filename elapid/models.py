...
class SDMMixin:
    ...
    ) -> Tuple[unumpy.uarray, unumpy.uarray, unumpy.uarray, unumpy.uarray, unumpy.uarray]:
    ...
    def partial_dependence_plot(
        ...
        ci_level: float = 0.95,
        **kwargs,
    ) -> Tuple[plt.Figure, unumpy.uarray]:
...
def maxent_alpha(raw: unumpy.uarray) -> float:
...
def maxent_entropy(raw: unumpy.uarray) -> float:
...
def maxent_raw_transform(engma: unumpy.uarray) -> unumpy.uarray:
...
def maxent_logistic_transform(engma: unumpy.uarray, entropy: float, tau: float = MaxentConfig.tau) -> unumpy.uarray:
...
def maxent_cloglog_transform(engma: unumpy.uarray, entropy: float) -> unumpy.uarray:
...
def format_occurrence_data(y: ArrayLike) -> ArrayLike:
    ...
    """
    if not isinstance(y, unumpy.uarray):
        y = unumpy.uarray(y)
